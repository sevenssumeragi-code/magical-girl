...
| `woke_renny` | C04 (`ch01_03`) | 寝ているレニィを起こした | `ch01_03` 後半の地の文が変化 |
| `reached_for_muni` | C05 (`ch01_04`) | ムニに手を差し出した | `ch01_04` 後半の地の文が変化 |
| `noticed_muni_slip` | C09 (`ch01_12`) | ムニの「にいちゃ……」を追及した | 伏線A回収時の演出強化 / `truth_material` 取得補助 |
| `stayed_with_muni` | C11 (`ch02_03`) | 眠るムニに付き添った（狩りに出なかった） | `ch02_03` 後半の地の文が変化 |
| `noticed_hair_color` | C12 (`ch02_04`) | レニィとムニの髪色の一致に触れた | 同上 |
| `stopped_jinpachi` | C15 (`ch02_09`) | ジンパチの単独行動を正面から止めた | `ch02_09` 後半の反応が変化 |
...
| `held_back_departed` | C30 (`ch04_12`) | 決別する相手を引き止めた | `ch04_12` 後半の地の文が変化 |
| `last_talk_renny` | C34 (`ch05_04`) | 最後の夜にレニィと話した | `ch05_04` の対話内容 |
| `last_talk_jinpachi` | C34 (`ch05_04`) | 最後の夜にジンパチと話した | 同上 |
| `last_talk_hyu` | C34 (`ch05_04`) | 最後の夜にヒュウと話した | 同上 |
